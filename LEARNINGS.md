...
- 5 of 9 UI tests (`testAllFiveTabsExist`, `testAutomationsAddMenu`, `testSettingsNavigationToSyncSettings`, `testSettingsShowsAllLinks`, `testTabSwitching`) fail pre-existing on the base commit — tab bar buttons "Automations" and "Settings" aren't found by identifier. Likely a timing/accessibility issue with SwiftUI `TabView` in the simulator. These are not regressions.
