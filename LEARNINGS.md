...
- `InsightItem` enum: when adding new cases, update `id`, `dataType`, `iconName`, and `message` computed properties on the enum itself. Don't duplicate icon/message logic in views or VMs — both `InsightCardView` and the widget bridge read from the same computed properties.
